...
네트워크 장치에 어떤 BPF 프로그램을 설치하면 패킷 수신 처리 초반에 실행한다. BPF와 XDP는 조만간 다른 글에서 다시 만난다.
