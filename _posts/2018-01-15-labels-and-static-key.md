...
객체에 `&`를 붙이면 객체의 주소이듯 레이블에 `&&`를 붙이면 [레이블의 주소](https://gcc.gnu.org/onlinedocs/gcc/Labels-as-Values.html)가 된다. 결과를 `void *` 타입 변수에 저장했다가 `goto *ptr;`처럼 사용할 수도 있다. 배열에 저장할 수도 있으니 상태 머신에서 점프 테이블로 사용하기에 좋다.
...
커널 소스 [Documentation/static-keys.txt](https://wariua.cafe24.com/wiki/Documentation/static-keys.txt)에 설명이 있다. 간단히 말해 특정 조건에서만 (가령 모니터링을 켰을 때만) 실행하는 루틴으로 향하는 브랜치 인스트럭션을 런타임에 바꿔치기하는 것이다. 평상시엔 브랜치 인스트럭션 자리에 no-op를 채워 두며, 그래서 실행이 직선으로 진행된다. 그러다가 런타임에 키 값을 바꾸면 (즉 키를 돌리면) 그 키를 사용하는 지점들을 모두 브랜치 하는 인스트럭션으로 바꾼다. 즉, 런타임에 메모리 상의 코드를 수정한다.

예를 들어 `perf` 프로그램으로 정보를 수집하는 동안에만 실행되는 루틴이 있다. 평상시에는 그 루틴이 없는 것처럼 돌다가 시스템 호출 등을 통해 동작을 바꾼다.
...
그리고 스케줄러에서 문맥 전환 때마다 다음 두 함수를 호출한다. `__perf_event_task_sched_{in,out}()` 안에 `perf`와 관련된 복잡한 내용이 들어 있다.
...
네트워크 장치에 어떤 BPF 프로그램을 설치하면 패킷 수신 처리 초반에 실행하는 코드이다. BPF와 XDP는 조만간 다른 글에서 다시 만난다.

커널의 정적 키 코드를 복붙하면 다음과 같다. x86-64 기준이다.
...
`arch_static_branch()`의 `asm goto` 코드에서는 no-op 인스트럭션(`0x0f,0x1f,...`)을 생성하고 나서 `__jump_table`이라는 섹션에 레이블 `1:`의 주소, 레이블 `l_yes`의 주소, `key`의 주소(+`branch` 값)을 추가한다. 이 세 값이 있으면 양방향으로 인스트럭션을 바꿀 수 있다.
...
.text 세그먼트의 오프셋 0x08과 0x0e를 위의 디스어셈블 코드에서 확인해 보면 각각 브랜치 위치와 점프 목적지인 걸 확인할 수 있다. 하지만 런타임에 0x08에 있는 인스트럭션을 바꾸는 것까지 따라해 볼 수는 없다. 텍스트 세그먼트에 쓰기를 하는 건 커널이나 되니까 가능한 일이기 때문이다. 췟.
